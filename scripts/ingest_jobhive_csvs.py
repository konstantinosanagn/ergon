...
don't brute-force-guess anything: we map each row directly to ergon's candidate schema and
...
Only the 8 ATSes ergon has providers for are ingested (their other ~18 — join.com,
...
# ATSes ergon has providers for (their CSV stem == our provider name for all 8).
SUPPORTED_ATSES = (
    "greenhouse",
    "lever",
    "ashby",
...
def row_to_candidate(ats: str, row: dict[str, str]) -> dict[str, object] | None:
    """Map one jobhive CSV row to a ergon candidate dict, or ``None`` if unmappable.
