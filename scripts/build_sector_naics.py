...
# NAICS 2-digit sector -> ergon 27-label vocab.
...
def main():
    ...
    if both:
        print(f"accuracy vs curated: {correct}/{len(both)} = {correct / len(both) * 100:.1f}%")
    # per-naics breakdown of accuracy
    from collections import Counter

    naics_n = Counter(out[c]["naics"] for c in both)
    naics_ok = Counter(out[c]["naics"] for c in both if out[c]["sector"] == curated[c]["sector"])
    print("\nper-NAICS accuracy on overlap (naics: correct/total -> vocab):")
    for code in sorted(naics_n, key=lambda c: -naics_n[c]):
        print(f"  {code}: {naics_ok[code]}/{naics_n[code]} -> {NAICS_VOCAB[code]}")
    ...
    FAMILY = {
        ...
        "22": {"Energy/Climate"},
        "21": {"Energy/Climate"},
        "53": {"RealEstate/PropTech"},
        "61": {"Education"},
        "71": {"Media/Entertainment", "Gaming"},
        "92": {"Government/Public"},
        "23": {"Manufacturing/Industrial", "RealEstate/PropTech"},
        "56": {"Consulting/Services"},
        "11": {"Other"},
        "55": {"Other"},
