...
from ergon.models import EmploymentType, JobLevel, RemoteType, SearchQuery  # noqa: E402
...
def main() -> None:
    pool = ergon.search(companies=COMPANIES, limit=400).jobs
    n = len(pool)
    ...
    print(
        f"  emp_type    : {pct(sum(j.employment_type is not EmploymentType.UNKNOWN for j in pool), n)}"
    )
    ...
    print(
        f"  country     : {pct(sum(bool(j.locations and j.locations[0].country) for j in pool), n)}"
    )
    ...
    checks: list[tuple[str, SearchQuery, object]] = [
        (
            "keywords='engineer'",
            SearchQuery(keywords="engineer"),
            lambda j: (
                "engineer" in (j.title or "").lower()
                or "engineer" in (j.description_text or "").lower()
            ),
        ),
        (
            "remote=True",
            ...
            lambda j: (
                j.remote in (RemoteType.REMOTE, RemoteType.HYBRID)
                or any(loc.is_remote for loc in j.locations)
                or j.remote is RemoteType.UNKNOWN
            ),
        ),
        ("level=SENIOR", SearchQuery(level=JobLevel.SENIOR), lambda j: j.level is JobLevel.SENIOR),
        (
            "employment_type=FULL_TIME",
            SearchQuery(employment_type=EmploymentType.FULL_TIME),
        ...
        (
            ...
            SearchQuery(country="United States"),
            lambda j: any(
                (loc.country or "").lower() == "united states"
                or "united states" in (loc.raw or "").lower()
                for loc in j.locations
            ),
        ),
        (
            "city='New York'",
            SearchQuery(city="New York"),
            lambda j: any(
                (loc.city or "").lower() == "new york" or "new york" in (loc.raw or "").lower()
                for loc in j.locations
            ),
        ),
        (
            "salary_min=200000 (excl unknown)",
            SearchQuery(salary_min=200000, include_unknown_salary=False),
            lambda j: (
                j.salary is not None and (j.salary.max_amount or j.salary.min_amount or 0) >= 200000
            ),
        ),
        (
            "salary_max=120000 (excl unknown)",
            SearchQuery(salary_max=120000, include_unknown_salary=False),
            lambda j: (
                j.salary is not None
                and (j.salary.min_amount or j.salary.max_amount or 1e9) <= 120000
            ),
        ),
        (
            "min_years=5 (excl unknown)",
            SearchQuery(min_years=5, include_unknown_years=False),
            lambda j: (j.years_experience_max or j.years_experience_min or 0) >= 5,
        ),
        (
            "max_years=2 (excl unknown)",
            SearchQuery(max_years=2, include_unknown_years=False),
