"""ergon MCP server — exposes the SDK as Model Context Protocol tools.

Run it (after ``pip install 'ergon[mcp]'``)::

    ergon-mcp            # stdio transport (for Claude Desktop / MCP clients)
...
The tools are thin adapters over the existing ``ergon`` API. ``search_jobs`` returns a
...
_INSTRUCTIONS = """\
ergon: free, unified job search over 30+ company ATS feeds + aggregators, with a daily
...
mcp = MCPServer("ergon", instructions=_INSTRUCTIONS)
