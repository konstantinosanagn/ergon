...
from .exceptions import ErgonError

app = typer.Typer(
    add_completion=False,
    no_args_is_help=True,
    help="ergon — unified job-fetching SDK CLI",
)
console = Console()
...
@app.command()
def version() -> None:
    """Print the ergon version."""
    console.print(f"ergon {__version__}")
...
def sources() -> None:
    ...
    table = Table(title="ergon providers")
...
def resolve(target: str) -> None:
    ...
    from .sync import Ergon

    try:
        resolution = Ergon().resolve(target)
    except (ErgonError, NotImplementedError, ImportError) as exc:
...
def search(
    ...
    semantic: bool = typer.Option(
        False,
        "--semantic",
        help="rank by meaning via embeddings (needs: pip install 'ergon[semantic]')",
    ),
