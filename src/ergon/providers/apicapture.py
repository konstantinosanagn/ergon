...
def _load_specs() -> dict[str, dict[str, Any]]:
    try:
        text = (files("ergon.registry.data") / "apicapture.json").read_text("utf-8")
...
def apply_token_to_spec(spec: dict[str, Any], value: str) -> dict[str, Any]:
    ...
    mutated, so a stale token can never leak across calls. See :mod:`ergon.token_store`."""
...
class ApiCaptureProvider(BaseProvider):
    ...
    async def fetch(self, token: str, query: SearchQuery, fetcher: AsyncFetcher) -> list[RawJob]:
        ...
        # the index). The browser is never on this path. See ergon.token_store.
