...
# Names of first-party provider modules under ergon.providers to import on startup.
_BUILTIN_MODULES = (
    "greenhouse",
    "lever",
    "ashby",
...
def load_builtins() -> None:
    ...
    for mod in _BUILTIN_MODULES:
        try:
            import_module(f"ergon.providers.{mod}")
        except ModuleNotFoundError:
            continue


def load_plugins() -> None:
    """Discover third-party providers via the ``ergon.providers`` entry-point group.
