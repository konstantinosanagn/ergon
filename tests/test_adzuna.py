...
from ergon.providers.adzuna import AdzunaProvider, _country_slug
...
def _configured(monkeypatch) -> None:
    ...
    monkeypatch.setattr("ergon.providers.adzuna.get_env", lambda k: creds.get(k))
...
async def test_fetch_skips_without_keys(monkeypatch) -> None:
    monkeypatch.setattr("ergon.providers.adzuna.get_env", lambda k: None)
    with respx.mock:
