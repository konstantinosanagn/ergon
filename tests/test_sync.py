...
Both the module-level ``search()`` and ``Ergon.search()`` default
...
from ergon.sync import Ergon, search
...
def test_ergon_search_defaults_max_last_seen_age_days_21(monkeypatch):
    captured = _capture_query(monkeypatch)
    Ergon().search("engineer")
    assert captured["query"].max_last_seen_age_days == 21


def test_ergon_search_include_stale_leaves_it_none(monkeypatch):
    captured = _capture_query(monkeypatch)
    Ergon().search("engineer", include_stale=True)
    assert captured["query"].max_last_seen_age_days is None


def test_ergon_search_explicit_value_preserved(monkeypatch):
    captured = _capture_query(monkeypatch)
    Ergon().search("engineer", max_last_seen_age_days=7)
    assert captured["query"].max_last_seen_age_days == 7
