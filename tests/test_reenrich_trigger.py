...
def test_version_bump_reenriches_employment_type_from_jd(monkeypatch, tmp_path):
    ...
    sid = "10"
    jid = make_job_id("greenhouse", sid)
    ...
    con = connect(fresh_prior)
    con.execute("PRAGMA foreign_keys = OFF")
    append_jobs(con, [stale], build_id="prior")
    con.commit()
    con.close()
    prior_db = tmp_path / "prior.sqlite"
    build_index_from_fresh_db(fresh_prior, prior_db, build_id="prior")
